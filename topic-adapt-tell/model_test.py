...
class SeqGAN():
	...
	self.load_ckpt = conf.load_ckpt
        self.load_seqgan_ckpt = conf.load_seqgan_ckpt
	...
	# argmax prediction
        _, predict_words_list_argmax, log_probs_action_picked_list_argmax, _, self.predict_mask_argmax \
                = self.generator_test(name='G', reuse=False)
        # tf.pack is deprecated by tf.stack
        # predict_words_argmax = tf.stack(predict_words_list_argmax)
        # self.predict_words_argmax = f.transpose(predict_words_argmax, [1,0]) # B,S
	# rollout = []
	# rollout_length = []
	...
	D_fake_vqa_softmax, D_fake_logits_vqa = self.discriminator(
                self.batch_size, 
                self.images, 
                tf.to_int32(self.predict_words_sample), 
                tf.to_int32(self.fake_length), 
                name="D", 
                reuse=True)

	D_right_vqa_softmax, D_right_logits_vqa = self.discriminator(
                self.batch_size, 
                self.images, 
                self.right_text, 
                self.right_length, 
	...
	D_wrong_vqa_softmax, D_wrong_logits_vqa = self.discriminator(
                self.batch_size, 
                self.images, 
                self.wrong_text,
                self.wrong_length, 
	...
	D_right_loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(
                    logits=D_right_logits_vqa, 
                    labels=tf.concat(
                        (tf.ones((self.batch_size,1)), tf.zeros((self.batch_size,1)), tf.zeros((self.batch_size,1))), 
                        1)))
	#       tf.concat(1,(tf.zeros((self.batch_size,1)), tf.ones((self.batch_size,1)), tf.zeros((self.batch_size,1))))))
	D_wrong_loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(
                    logits=D_wrong_logits_vqa,
                    labels=tf.concat(
                        (tf.zeros((self.batch_size,1)), tf.ones((self.batch_size,1)), tf.zeros((self.batch_size,1))),
                        1)))
	#       tf.concat(1,(tf.zeros((self.batch_size,1)), tf.zeros((self.batch_size,1)), tf.ones((self.batch_size,1))))))
	D_fake_loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(
                    logits=D_fake_logits_vqa,
                    labels=tf.concat(
                        (tf.zeros((self.batch_size,1)), tf.zeros((self.batch_size,1)), tf.ones((self.batch_size,1))),
                        1)))
    ...
    def discriminator(self, batch_size, images, text, length, name="discriminator", reuse=False):
	    ...
	    length_index = tf.where(
            ...
            # images embedding
            # images_emb = tf.matmul(images, theme_embedding) + images_b # B,H
	...
	with tf.variable_scope(name):
		...
		for j in range(self.max_words-step-1):
		    ...
		    # add 1e-8 to prevent log(0) 
                    ...
                    print 'sample_words shape:', sample_words.shape
                    print 'predict_words shape:', predict_words
    ...
    def visualize(self, themes_list, captions_list=None):
        ...
        def predict_words(themes):
            ...
            mask = tf.constant(True, "bool", [self.batch_size])
            log_probs_action_picked_list = []
            predictWords = np.zeros([1, self.max_words+1], dtype=int)
            predict_mask_list = []
            for j in range(self.max_words+1):
                if j == 0:
                    with tf.device("/cpu:0"):
                ...
                if j > 0:
                    ...
                    mask_out_word = tf.where(mask, sample_words,
                                                tf.constant(self.NOT, dtype=tf.int64, shape=[self.batch_size]))
                    # print mask_out_word.eval()
                    predictWords[0, j] = mask_out_word.eval()[0]
	    samples = self.dataset.decode(
                    predictWords, 
                    type='string', 
                    remove_END=True)[0]
            return samples
